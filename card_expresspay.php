...
class plgVMPaymentCard_Expresspay extends vmPSPlugin {
    ...
    protected function getVmPluginCreateTableSQL() {
			return $this->createTableSQL('Payment Card ExpressPay Table');
    }
    ...
    function plgVmConfirmedOrder($cart, $order) {
		...
		$url .= "/v1/cardinvoices?token=" . $method->token;

		$order_id = $order['details']['BT']->virtuemart_order_id; //virtuemart_order_id or order_number

		$info = nl2br($method->info, true);
		$info = str_replace("##order_id##", $order_id, $info);

		$info = empty($info) ? "Order Number " . $order_id : $info;

		$this->log_info("Info", $info);
        ...
        $request_params = array(
			...
			"Currency" 		=> $currency,
			"Info"			=> $info,
			"ReturnUrl" 	=> JROUTE::_(JURI::root() . 'index.php?option=com_virtuemart&view=pluginresponse&task=pluginresponsereceived&pm=' . $order['details']['BT']->virtuemart_paymentmethod_id . '&accountno=' . $order_id ),
			"FailUrl"		=> JROUTE::_(JURI::root() . 'index.php?option=com_virtuemart&view=pluginresponse&task=pluginresponsereceived&pm=' . $order['details']['BT']->virtuemart_paymentmethod_id . '&accountno=' . $order_id .'&status=canceled'  )
        );
        ...
        if(isset($response->CardInvoiceNo)) {
			...
			$url = ( $method->test_mode == 1 ) ? $method->url_sandbox_api : $method->url_api;
			$url .= "/v1/cardinvoices/" .$response->CardInvoiceNo . '/payment?token=' . $method->token;
			...
			if($is_use_signature)
				$url .= "&signature=" . $this->compute_signature_get_form_url($request_params, $secret_word);
				
			$this->log_info('plgVmConfirmedOrder', 'Send POST request; CardInvoiceNo - ' . $response->CardInvoiceNo . '; URL - ' . $url);
			...
			if(isset($response->FormUrl)){
				...
				$this->log_info("Форма оплаты", $returnUrl);

				$app = JFactory::getApplication();
				$app->redirect( $returnUrl , false);
			}
			else{

				$this->log_error('plgVmConfirmedOrder', $response->Error['Code']);

				$this->processFailOrder($cart, $order, $this->template, $method->payment_name, $method->status_canceled, $order_id);
	...
	private function processFailOrder($cart, $order, $template, $payment_name, $status_canceled, $order_id)
	{
			$this->log_info('plgVmConfirmedOrder', 'End request for add invoice');
			$this->log_info('plgVmConfirmedOrder', 'Initialization render fail page; ORDER ID - ' . $order_id);
			...
			$this->processConfirmedOrderPaymentResponse(false, $cart, $order, $template, $payment_name, $status_canceled);
	}
...
EOD;
	...
	function plgVmOnShowOrderBEPayment($virtuemart_order_id, $virtuemart_payment_id) {
		...
		$html .= $this->getHtmlRowBE('CARD_EXPRESSPAY_PAYMENT_NAME', $paymentTable->payment_name);
		$html .= '</table>' . "\n";
    ...
    function plgVmOnPaymentResponseReceived(&$html) {
		...
		$order_number = $payment_data['accountno'];
		$order_status = $payment_data['status'];
		...
		{
			$html = "Оплата заказа номер " . $order_number . " не произведена из-за непредвиденной ошибки.";
		}
		else if( $order_status == 'canceled')
		{
			$html = $method->message_fail;
			$html = str_replace('##order_id##', $order_number, $html);
		}
		else
		{
			$html = $method->message_success;
			$html = str_replace('##order_id##', $order_number, $html);
		}
    ...
    function log($name, $type, $message) {
    	...
    	$file_name = 'Card_expresspay/express-pay-' . date('Y.m.d') . '.php';
